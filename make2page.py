...
SRC = r'C:\Users\irgir\belumbayar-repo\index.html'
DST = r'C:\Users\irgir\belumbayar-repo\index_new.html'

with open(SRC, 'r', encoding='utf-8') as f:
    old = f.read()

# Extract data and config blocks
# Exact file structure (verified):
# DATA: const PBB_DATA = [...] ; ends at char 104761
# DUSUN_RW + getDusun functions: 104805 to 105222
# STATE variables: 105222 to 106441
data_start = old.find('const PBB_DATA')
data_end = old.find('];', 24358) + 2
dusun_s = old[104805:105222]
state_s = old[105222:106441]
data_s = old[data_start:data_end]
pat = '{"nama"'
print(f"Data: {data_end-data_start} chars, entries: {data_s.count(pat)}")
print(f"Dusun: {len(dusun_s)} chars, has getDusun: {'function getDusun' in dusun_s}")
print(f"State: {len(state_s)} chars")

def get_fn(name):
    idx = old.find('function ' + name)
    if idx < 0:
        for p in ['let ', 'const ']:
            idx = old.find(p + name)
            if idx >= 0: break
    ...
    brace = old.find('{', idx)
    bc, s = 0, False
    for i in range(brace, len(old)):
        if old[i] == '{': bc += 1; s = True
        elif old[i] == '}':
            bc -= 1
            if s and bc == 0: return old[idx:i+1]
    return ''
...
fns = {n: get_fn(n) for n in fnames}
...
.nv.ak{color:#fff;border-bottom-color:var(--e);background:rgba(255,255,255,.08)}
...
.bf.hi{background:linear-gradient(90deg,#1a5c3a,#2e8b57)}
...
.kp .tbl2{border-collapse:collapse;margin-bottom:14px;width:auto}
.kp .tbl2 td{padding:2px 0;font-size:10.5pt;vertical-align:top}
.kp .tbl2 td:nth-child(2){padding:0 8px}
...
.kp .ttd3{display:flex;justify-content:flex-end;margin-top:20px}
...
.iw .tc2{display:flex;flex-wrap:wrap;gap:4px;margin-top:6px}
.ch3{background:#fff3cd;color:#7a5c00;padding:2px 9px;border-radius:8px;font-size:0.75rem;font-weight:600}
.ch3.tot{background:#fee2e2;color:#991b1b}
...
@media print{body>*{display:none!important}#print-area{display:block!important}@page{margin:2cm;size:A4 portrait}.sc{font-family:'Times New Roman',serif;font-size:12pt;line-height:1.6;page-break-after:always}.sc:last-child{page-break-after:avoid}.sc .kop{text-align:center;border-bottom:3px double black;padding-bottom:10px;margin-bottom:16px}.sc .kop-nama{font-size:14pt;font-weight:bold;text-transform:uppercase}.sc .kop-sub{font-size:11pt}.sc .judul{text-align:center;font-size:13pt;font-weight:bold;text-decoration:underline;text-transform:uppercase;margin:12px 0 4px}.sc .nomor{text-align:center;font-size:11pt;margin-bottom:18px}.sc .tbl2{border-collapse:collapse}.sc .tbl2 td{padding:2px 0;font-size:11pt;vertical-align:top}.sc .tbl2 td:nth-child(2){padding:0 8px}.sc .para{margin-bottom:12pt;font-size:11pt;text-align:justify}.sc .nominal{text-align:center;font-size:13pt;font-weight:bold;margin:12pt 0 4pt}.sc .nominal-kecil{text-align:center;font-size:10pt;color:#333;margin-bottom:14pt}.sc .ttd3{display:flex;justify-content:flex-end;margin-top:24pt}.sc .ttd-box{text-align:center;min-width:210px}.sc .ttd-jabatan{font-weight:bold}.sc .ttd-spasi{height:70pt}.sc .ttd-nama{font-weight:bold;text-decoration:underline}}
...
<nav class="nb">
  ...
  <div class="nt">
    <div class="nv ak" id="tab-dashboard" onclick="showPage('dashboard')">
    ...
    <div class="pt">
      <div class="th">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
          ...
          <button class="bc2" onclick="cetakBatch('undangan')">&#9993; Undangan</button>
        </div>
      </div>
      <div class="ts">
        <table>
...
<div class="ov" id="modal-pb"><div class="md">
  <div class="mh"><h3>&#128196; Surat Pemberitahuan PBB-P2</h3><button onclick="tutupModal('pb')">x</button></div>
  <div class="mb">
    <div class="mfc">
  ...
  <div class="mf">
    ...
    <button class="bcf" onclick="docetak('pb')">&#128424; Cetak Surat</button>
  </div>
</div></div>

<!-- MODAL UNDANGAN -->
<div class="ov" id="modal-und"><div class="md">
  <div class="mh und"><h3>&#9993; Surat Undangan PBB-P2</h3><button onclick="tutupModal('und')">x</button></div>
  <div class="mb">
    <div class="mfc">
  ...
  <div class="mf">
    ...
    <button class="bcf" onclick="docetak('und')">&#128424; Cetak Surat</button>
  </div>
</div></div>
...
function showPage(page) {
  document.querySelectorAll('.pg').forEach(function(p){p.classList.remove('ak')});
  document.querySelectorAll('.nv').forEach(function(t){t.classList.remove('ak')});
  document.getElementById('page-' + page).classList.add('ak');
  document.getElementById('tab-' + page).classList.add('ak');
  if (page === 'dashboard') {
    initDashboard();
  } else {
    applyFilter();
...
function initDashboard() {
  var total = PBB_DATA.reduce(function(s,d){return s+d.total},0);
  var totalWp = PBB_DATA.length;
  ...
  // Hero breakdown - 5 columns
  var dStats = [];
  for (var d = 0; d <= 4; d++) {
    var subset;
    if (d === 0) subset = PBB_DATA;
    else if (d === 4) subset = PBB_DATA.filter(function(x){return x.rw === '0'});
    else subset = PBB_DATA.filter(function(x){return getDusun(x.rw) === d});
    dStats.push({count: subset.length, total: subset.reduce(function(s,x){return s+x.total},0)});
  }
  var labels = ['Semua','Daven I','Daven II','Daven III','Luar'];
  var brHtml = '';
  for (var i = 0; i < dStats.length; i++) {
    var ds = dStats[i];
    brHtml += '<div class="hbs">' +
      '<div class="hbsn">' + rp(ds.total) + '</div>' +
      '<div class="hbsl">' + labels[i] + '</div>' +
      '<div class="hbsl">' + ds.count + ' WP</div>' +
    '</div>';
  }
  document.getElementById('hero-breakdown').innerHTML = brHtml;

  // Daven cards
  var cardLabels = ['&#127968;&#65039; Semua','&#127807; Daven I','&#127807; Daven II','&#127807; Daven III','&#127806; Luar'];
  var cardRws = ['RW 1-7','RW 01 - RW 02','RW 03 - RW 06','RW 04 - RW 05 - RW 07','RW 0'];
  var maxTotal = Math.max.apply(null, dStats.map(function(ds){return ds.total}));
  var cardsHtml = '';
  for (var d = 0; d <= 4; d++) {
    var ds = dStats[d];
    var pct = maxTotal > 0 ? (ds.total / maxTotal * 100).toFixed(1) : 0;
    var extra = d === 4 ? ' lu' : '';
    cardsHtml += '<div class="dc' + extra + '" id="dcard-d' + d + '" onclick="gotoData(' + d + ')">' +
  ...
  setTimeout(function() {
    for (var d = 0; d <= 4; d++) {
      var ds = dStats[d];
      var pct = maxTotal > 0 ? (ds.total / maxTotal * 100).toFixed(1) : 0;
      var bar = document.getElementById('dbar-d' + d);
  ...
  // RW Chart
  var rwMap = {};
  PBB_DATA.forEach(function(d){
    var r = d.rw || '0';
    if (!rwMap[r]) rwMap[r] = 0;
    rwMap[r] += d.total;
  });
  var rwLabels = Object.keys(rwMap).sort(function(a,b){return +a-+b});
  var maxRw = Math.max.apply(null, Object.values(rwMap));
  var colors = ['hi','e','m','u','a2','hi','e','m','u','a2'];
  var rwHtml = '';
  for (var i = 0; i < rwLabels.length; i++) {
    var r = rwLabels[i];
    var v = rwMap[r];
    var w = maxRw > 0 ? (v / maxRw * 100).toFixed(1) : 0;
    rwHtml += '<div class="br">' +
      '<div class="bl">RW ' + r + '</div>' +
      '<div class="bt"><div class="bf ' + colors[i % colors.length] + '" style="width:' + w + '%">' + rp(v) + '</div></div>' +
      '<div class="bv">' + rp(v) + '</div>' +
    '</div>';
  }
  document.getElementById('chart-rw').innerHTML = rwHtml;

  // Year Chart
  var yearMap = {};
  PBB_DATA.forEach(function(d){
    Object.entries(d.tunggakan).forEach(function(entry){
      var t = entry[0], v = entry[1];
      if (!yearMap[t]) yearMap[t] = 0;
      yearMap[t] += v;
    });
  });
  var yearLabels = ['2020','2021','2022','2023','2024','2025'];
  var yearVals = yearLabels.map(function(y){return yearMap[y] || 0});
  var maxYear = Math.max.apply(null, yearVals.concat([1]));
  var yrHtml = '';
  for (var i = 0; i < yearLabels.length; i++) {
    var y = yearLabels[i];
    var v = yearVals[i];
    var w = (v / maxYear * 100).toFixed(1);
    yrHtml += '<div class="br">' +
      '<div class="bl">' + y + '</div>' +
      '<div class="bt"><div class="bf ' + colors[i % colors.length] + '" style="width:' + w + '%">' + rp(v) + '</div></div>' +
      '<div class="bv">' + rp(v) + '</div>' +
  ...
  document.getElementById('chart-year').innerHTML = yrHtml;

  // Top 10
  var sorted = PBB_DATA.slice().sort(function(a,b){return b.total-a.total}).slice(0,10);
  var ranks = ['g','s','b','n','n','n','n','n','n','n'];
  var topTotal = sorted.reduce(function(s,d){return s+d.total},0);
  document.getElementById('top-count').textContent = 'Total 10 besar: ' + rp(topTotal);
  var topHtml = '';
  for (var i = 0; i < sorted.length; i++) {
    var d = sorted[i];
    topHtml += '<div class="ti">' +
...
JS = JS_PART1 + JS_PART2

FULL = HTML + JS + "\n</script>\n</body>\n</html>\n"

print("Generated: " + str(len(FULL)) + " chars")
...
with open(DST, 'r', encoding='utf-8') as f:
    check = f.read()
print("Check: showPage=" + str('function showPage' in check))
print("Check: initDashboard=" + str('function initDashboard' in check))
print("Check: hero-total=" + str('id="hero-total"' in check))
print("Check: dusun-grid=" + str('id="dusun-grid"' in check))
print("Check: chart-rw=" + str('id="chart-rw"' in check))
print("Check: top-list=" + str('id="top-list"' in check))
print("Check: page-data=" + str('id="page-data"' in check))
print("Check: page-dashboard=" + str('id="page-dashboard"' in check))
print("ALL OK" if all([
    'function showPage' in check,
    'function initDashboard' in check,
    'id="hero-total"' in check,
    'id="dusun-grid"' in check,
    'id="chart-rw"' in check,
    'id="top-list"' in check,
    'id="page-data"' in check,
    'id="page-dashboard"' in check,
